		...
		for(int h = 0; h < maxHeigth; ++h) {
			for (int r = 0; r < maxRow; ++r) {
				...
				for (int c = 0; c < maxCol; ++c) {
					...
					if (num == 1)
						queue.add(new int[] { r, c, h }); // 익은 토마토
					else if (num == 0)
		...
		while (!queue.isEmpty()) {
			...
			while (!goodTomatoQueue.isEmpty()) {
				...
				for (int i = 0; i < 4; ++i) {
					...
					if (map[nr][nc][pos[2]] == -1 || map[nr][nc][pos[2]] == 1) continue; // 토마토가 들어있지 않거나, 이미 익은 토마토인 경우 pass
					
					//익지 않은 토마토들을 익힌 뒤 넣어준다 .
					map[nr][nc][pos[2]] = 1; 
					sadTomatoCnt--;
					queue.add(new int[] { nr, nc, pos[2] });
				}
				
				for(int i = 0; i < 2; ++i) {
					int nh = pos[2] + (i%2 == 0 ? -1 : 1);
					
					if(nh < 0 || nh >= maxHeigth) continue;
					if (map[pos[0]][pos[1]][nh] == -1 || map[pos[0]][pos[1]][nh] == 1) continue;
					
					map[pos[0]][pos[1]][nh] = 1; 
					sadTomatoCnt--;
					queue.add(new int[] { pos[0], pos[1], nh });
				}
			}
		}
